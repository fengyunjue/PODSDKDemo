...
- (CGFloat)_updateMagnifier:(YYTextMagnifier *)mag {
    ...
    CGPoint captureCenter = [self kf5_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
...
- (void)showMagnifier:(YYTextMagnifier *)mag {
    ...
    CGPoint center = [self kf5_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
...
- (void)_updateSelectionGrabberDot:(YYSelectionGrabberDot *)dot selection:(YYTextSelectionView *)selection{
    ...
    if (selection.hostView.clipsToBounds == YES && dot.kf5_visibleAlpha > 0.1) {
        CGRect dotRect = [dot kf5_convertRect:dot.bounds toViewOrWindow:self];
    ...
    CGPoint center = [dot kf5_convertPoint:CGPointMake(CGRectGetWidth(dot.frame) / 2, CGRectGetHeight(dot.frame) / 2) toViewOrWindow:self];
